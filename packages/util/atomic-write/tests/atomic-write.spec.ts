...
// The blocked-rename fallback needs the rename call to fail on demand, and
// the lock-exclusion test needs the .lock writeFile to fail; the module
// namespace is not configurable under ESM, so the whole builtin is wrapped
// and both get mocks that delegate to the real implementation.
