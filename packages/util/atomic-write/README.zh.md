...
- **rename 被阻塞时回退为 unlink+rename**：当目标文件被其他进程以不共享删除的方式持有（`EPERM`/`EBUSY`/`EACCES`——Windows 上常见于第二个 harness 实例的 watcher、编辑器或杀毒扫描器），会先删除目标再重试一次 rename，避免持有中的目标让一次设置写入静默丢失。目录目标永远不会被回退路径替换。
- 自动创建父目录；任何失败都会移除临时文件并重新抛出该失败；读取方只会观察到旧内容或完整的新内容——阻塞回退造成的短暂缺失除外。
