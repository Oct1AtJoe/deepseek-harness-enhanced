...
`[text]` is the absence of a declaration rather than a guess at the endpoint, which is why the fallback here is conservative where the capacity fallbacks are merely plausible. Nothing interrogates a gateway for what it accepts, and the two wrong answers do not cost the same: under-claiming never refuses the message — a model whose modalities omit `image` still accepts the attachment, rendered as a text placeholder naming the durable image (the same shape `llm-deepseek` renders, so the model learns an image is present and can delegate actual reading to a vision-capable subagent) — while over-claiming admits pixels the provider then rejects mid-turn, after the message is durable, which leaves the session repeating a request that cannot succeed.
