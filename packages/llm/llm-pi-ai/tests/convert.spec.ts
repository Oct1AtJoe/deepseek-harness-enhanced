...
describe('toPiContext', () => {
  ...
  it('renders structured image history as a placeholder when no durable resolver is supplied', () => {
    const context = toPiContext({
    ...
    expect(JSON.stringify(context))
      .toContain(`[image attachment sha256:${'b'.repeat(64)} (image/png, 1x1px, 1 bytes)]`)
  })
