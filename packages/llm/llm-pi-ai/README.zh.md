...
`[text]` 是「尚未声明」，而不是对端点的猜测——这也是为什么这里的回退值取保守值，而两个容量回退值只是取一个说得过去的值。这里没有任何环节会去询问网关实际接受什么，而两种猜错的代价并不对等：少声明**不会拒绝消息**——模态中不含 `image` 的模型仍接受附件，图片块渲染为点名 durable 图片的文本占位符（与 `llm-deepseek` 渲染的形态一致，模型得知有图、可把实际识读委派给视觉子智能体）——而多声明会接纳像素、再由提供方在轮次中途拒绝，此时消息已经持久化，会话便会不断重复一个不可能成功的请求。
