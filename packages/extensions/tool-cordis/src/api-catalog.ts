...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  {
    ...
    methods: [
      ...
      {
        signature: 'modelEffort(provider: string, model: string): string | undefined',
        description: 'Read the remembered effort for one route, if any.',
        parameters: [{ name: 'provider', description: 'provider route.' }, { name: 'model', description: 'provider-owned model id.' }],
        returns: 'the remembered effort, or undefined.',
      },
      {
        signature: 'async saveModelEffort(provider: string, model: string, effort: string | undefined): Promise<void>',
        description: 'Record or clear the remembered effort for one route. A deployment without a settings provider keeps its composition entry.',
        parameters: [{ name: 'provider', description: 'provider route.' }, { name: 'model', description: 'provider-owned model id.' }, { name: 'effort', description: 'the effort to remember, or undefined to clear it.' }],
        returns: 'fulfillment after the optional settings write settles.',
      },
    ],
  },
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'setInvocationOverride(name: string, policy: SkillInvocationPolicy | undefined): void',
        description: 'Set or clear a runtime invocation override for one skill name. The override wins over every provider-declared policy at list and get projection; passing `undefined` restores the provider policy. A name with no catalog entry is accepted and applies once such a skill appears. Overrides live in this registry\'s global layer only.',
        parameters: [{ name: 'name', description: 'kebab-case skill name.' }, { name: 'policy', description: 'effective invocation policy, or `undefined` to clear.' }],
      },
    ],
  },
  {
