...
// error(4s) �?disappointed(6s) 瞬发（任务失败与请求错误同一负面窗口，�?10s）；新会�?�?welcome�?// 工作态累加活跃时长�?// 安全�?interact 校验跨源（CSRF）；body 上限 1KB；assets 路径净化拒�?`\` 段（Windows 穿越）�?// 持久化：状态存 <dshHome>/data/whale-girl/state.json�?tmp + rename 原子写，1s 防抖�?// 事件记账时落盘；disable 时末次落盘）
...
// 路由端点 re-export（来�?src/routes.mjs；保持既有导出面）
...
export function apply(ctx) {
  ...
  // 可能缺失；缺失时回退 DEFAULTS（插件照常跑，只是无用户配置面）�?  // configRef 是消费端唯一读取面（Node half 的窗口时长等）；configRevision �?  // /state 下发，客户端以此门控「配置变化才重应用」
  ...
  // 拉最�?/state——回合完成庆�?欢迎/思考陪伴不再等下一个轮询周期（默认 3s）�?  // 轮询保留兜底（SSE 断线/不可用时宠物照常跑，EventSource 内建自动重连）�?  // 连接管理：res 写入失败（断连）即从集合移除；close 时清理（心跳一并停）
  ...
  // 瞬发窗口（welcome > error > disappointed；celebrate 由任务派生——事�?+ 轮询两源）
  let errorUntil = 0
  ...
  // 每会话活动账本（/sessions 端点）：sessionId �?{ id, title, activity, since }�?  // 事件驱动更新（session/event 回调）；sessions 服务缺席时降级为仅事件视图
  ...
  // 每会话活动快照（/sessions 端点）：事件视图为主，sessions 列表兜底—�?  // 未在事件流出现的会话（如插件加载前已存在）用列表事件日志补标题�?  // header.createdAt �?since；列表缺席时只返回事件视图（宠物照常跑）�?  // 列表可用时清理由已结束会话（不再出现在列表）的视图—�?会话结束后框消失"
  ...
  // 账本信号：celebrate（任务完�?升级）、levelUp（升级）、failure（失败）、session（新会话/续接）�?  // 订阅者回�?(signal, payload)；订阅者异常隔离（不影响宠物本体）
  const signals = createSignals()
  const emitSignal = signals.emit

  // 派生活动 + 事件记账（积累）：完�?+XP/称号/回忆；失败计数；工作态累加活跃时长
  const activity = () => {
    const now = Date.now()
    ...
    // 账本记账�?XP/失败计数/回忆）已迁入 ctx.jobs.onJobDone 事件驱动—�?    // 页面关闭/轮询缺席时任务终态不漏记；此处只保留展示（working/burst）与活跃时长
  if (derived.working) {
      state = recordActive(state, now - lastActiveCheck, now).state
      scheduleSave()
    }
    lastActiveCheck = now
    // 任务失败与请求错误同一负面窗口：error(ERROR_MS) �?disappointed(尾段 DISAPPOINTED_MS)�?    // 窗口�?max：同一窗口内多次失�?错误只延长不缩短（越挫越勇不因并发被吞）
  ...
  let name = derived.working ? 'working' : 'idle'
    ...
    // 绝对截止时间�?/state 成为非消费式快照：Web 与外部伴侣可同时观察同一回合完成
    return {
      name,
      until,
      sessionThink,
  ...
  // 通道），不再�?entry 注入页面�?811 bundle 形态）
  ...
  ctx.effect(() => {
    const disposers = [
      ...
      // 2026-08-15-pet-service-namespace 决策记录）�?      // 不暴露任何写面（账本语义�?whale-girl 独占，防第三方破坏积累不变量）
  ctx.provide('kanye-pet.pet', {
        snapshot: () => ({ pet: state, activity: activity() }),
        onSignal: (fn) => signals.subscribe(fn),
      }),
      // 事件驱动记账（F1）：任务终态恰回调一次，与浏览器轮询解耦—�?      // GUI 关闭期间完成/失败的任务也入账（此前靠轮询观察 running 翻转，漏记窗口大）�?      // killed（用户取消）中性：不计 XP、不记失败、不写回忆（F4 语义）
  ctx.jobs.onJobDone((snapshot) => {
        ...
        if (snapshot.status === 'completed') {
          ...
          // 与轮询翻转的 celebrate �?max 不叠加（CELEBRATE_MS �?BURST_MS）
    celebrateUntil = Math.max(celebrateUntil, now + configRef.celebrateMs)
      ...
      ctx.on('agent/request-error', () => {
        ...
        // 避免一次坏任务多次请求错误刷出「越挫越勇」称号、回忆里出现虚假的「任务失败」�?        // 窗口与任务失败统一：error(ERROR_MS) �?disappointed(尾段)
  const now = Date.now()
      ...
      ctx.on('agent/session-start', (payload) => {
        const now = Date.now()
        // source 区分新会话（startup）与续接/延续（resume/compact/clear）——XP 不同�?        // 新会�?+5 + 计数 + welcome；续�?+2 不计数不 welcome（避免切换即欢迎的噪音）
  ...
  ctx.on('session/event', (session, event) => {
        ...
        // （titleFromLog 从列�?meta/事件日志取标题，since �?header.createdAt）
  const known = sessionViews.get(id)
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          try {
            ...
            // 只读配置端点：返回解析后的体验层配置（客户端�?configRevision 拉取）�?            // 写路径只有用户设置（settings 服务/文件）——插件不自建写面
  ...
  if (isCrossOrigin(req.headers, req.headers.host)) {
      ...
      // �?/interact 同级安全面（跨源校验 + body 上限）。见 src/presence.mjs 契约
  webServer.register({
        kind: 'exact',
        path: PRESENCE_PATH,
        handler: async (req, res) => {
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            // 内容不可变。发布契约：改图必须改文件名或角�?id——immutable 会滞留「同�?            // 被替换」的旧图（替代原 no-cache 的「替换同�?sheet 须重新校验」防线）
  const cacheCtrl = rel === 'manifest.json' ? 'no-cache, must-revalidate' : 'public, max-age=31536000, immutable'
      ...
      // 降到单次往返）。心�?25s 注释行防代理/网关空闲断开；close 清理连接与心跳�?      // 广播失败（断连）�?broadcastEvent �?try/catch 移除连接，不阻塞事件处理
  webServer.register({
        kind: 'exact',
        path: EVENTS_PATH,
        handler: async (req, res) => {
