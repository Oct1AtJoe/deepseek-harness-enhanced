...
(() => {
  ...
  let settingsCharacter = null // 来自 DSH settings，覆盖 manifest.default
  ...
  wg.onSnapshot((snap) => {
    snapshot = snap
    // 角色跟随 DSH settings（实时切换）
    if (snap?.character && snap.character !== characterId) {
      settingsCharacter = snap.character
      void switchCharacter(snap.character)
    }
    renderBadge()
  })
  wg.onReply((reply) => {
