...
import { SubagentComposerAction, type SubagentComposerInjected } from './SubagentComposerAction.tsx'
import {
...
export function apply(ctx: ClientContext): void {
  ...
  // conversation.input.right list seat). Clicking opens the sidebar's
  // subagent tab through the dsh-better-sidebar service, pinned to the
  // right panel (pane: 'right') so it surfaces in the side card even when
  // the user was last working in the collapsed bottom panel; the seat stays
  // unregistered-friendly (the inject reads the optional service at
  // click time, so the button degrades to a no-op without the plugin).
  ctx.slots.inject('conversation.input.right', () => ctx.slots.register({
    name: 'conversation.input.right',
    id: 'subagent-activity',
    order: 10,
    locale: NS,
    inject: (_sessionId: SessionId): SubagentComposerInjected => ({
      openSubagentTab: () => {
        const betterSidebar = ctx.get('betterSidebar') as {
          openTab: (seed: { type: string; pane?: 'right' }) => void
        } | undefined
        betterSidebar?.openTab({ type: 'subagent', pane: 'right' })
      },
    }),
