...
Local customization fork of the official `@deepseek-ai/dsh-client-ui-subagent` (see [CUSTOM-FEATURES.md](../../../CUSTOM-FEATURES.md) #4). The official package stays untouched so upstream syncs never conflict; this package is mounted instead and the official row is disabled in the user profile patch.

Web subagent feature owner: contributes the lazily expandable catalog tree to `conversation.session.header.actions`, reason-specific read-only replacements to the conversation composer chain, and the existing `@` reference source to `ctx.inputTriggers`. Unlike the official package, it no longer registers the `conversation.view` ring's subagent tab (the top tab is gone); the composer tool-row button (`conversation.input.right`) instead opens the sidebar subagent tab through the optional `ctx.betterSidebar` service (`openTab({ type: 'subagent', pane: 'right' })`, provided by the user-level `dsh-better-sidebar` plugin). `pane: 'right'` pins the tab to the right panel (the plugin's service patch, see CUSTOM-FEATURES.md #4) so it surfaces even when the bottom panel is collapsed. Without that service the button is a no-op.
