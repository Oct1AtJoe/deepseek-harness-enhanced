...
import {
  SubagentComposerAction, type SubagentComposerInjected,
} from '../src/client/SubagentComposerAction.tsx'
import {
...
describe('apply', () => {
  ...
  it('drops the top view tab and routes the composer button to the sidebar subagent tab', async () => {
    const { ctx } = await fullBench(FAMILY)
    // No conversation.view occupant: the plugin no longer contributes a top tab.
    expect(ctx.slots.entries('conversation.view')).toHaveLength(0)
    // The composer tool-row seat still hosts the activity button, whose
    // injected action opens the dsh-better-sidebar subagent tab pinned to
    // the right panel (never the collapsed bottom one).
    const entry = ctx.slots.entries('conversation.input.right')
      .find(candidate => candidate.component === SubagentComposerAction)!
    expect(entry).toBeTruthy()
    const openTabs: { type: string; pane?: 'right' }[] = []
    ctx.provide('betterSidebar', {
      openTab: (seed: { type: string; pane?: 'right' }) => { openTabs.push(seed) },
    } as never)
    const injected = (entry.inject as unknown as (id: SessionId) => SubagentComposerInjected)(sid('parent'))
    injected.openSubagentTab()
    expect(openTabs).toEqual([{ type: 'subagent', pane: 'right' }])
  })
})
