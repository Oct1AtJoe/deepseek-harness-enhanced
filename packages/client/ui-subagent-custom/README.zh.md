...
官方 `@deepseek-ai/dsh-client-ui-subagent` 的本地定制 fork（见 [CUSTOM-FEATURES.md](../../../CUSTOM-FEATURES.md) #4）。官方包保持不动，上游同步零冲突；本包替代挂载，官方行在用户 profile patch 中禁用。

Web subagent 功能 owner：向 `conversation.session.header.actions` 贡献可懒加载展开的目录树，向会话编辑器链贡献按原因区分的只读替代呈现，并保留注册到 `ctx.inputTriggers` 的既有 `@` 引用 source。与官方包不同，本包不再注册 `conversation.view` 环的子智能体 tab（顶部 tab 已移除）；输入区按钮（`conversation.input.right`）改为通过可选服务 `ctx.betterSidebar` 打开侧边栏子智能体 tab（`openTab({ type: 'subagent', pane: 'right' })`，由用户级 `dsh-better-sidebar` 插件提供）。`pane: 'right'` 把 tab 固定到右侧面板（该插件的服务补丁，见 CUSTOM-FEATURES.md #4），即使底部栏折叠也能浮现。该服务缺席时按钮点击无效。
