...
Web GUI 的极光（aurora）与星云（nebula）科技主题，注册到官方 [`dsh-client-ui-theme`](../ui-theme/README.zh.md) 主题注册表。浏览器插件注入主题服务并调用 `ctx.theme.register(...)` 注册两个定义；注册表快照随后驱动「外观」行的色块与 presenter，与内置主题对完全一致。
...
无。本插件只向浏览器主题注册表注册 CSS token 集；它不发起提示词、工具、消息或 provider 请求，也不触碰会话日志。
