...
describe('ui-theme apply', () => {
  ...
  it('holds a valid preference whose theme registers later (boot ordering)', async () => {
    const b = await bench()
    // Simulate the durable settings delivering a preference for a theme the
    // registrant plugin (ui-theme-custom) has not applied yet.
    b.setHostPreference('nebula')
    b.ctx.remote.$dispatch('settings/document-updated', [THEME_SETTINGS_NAMESPACE, 0])
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const theme = b.ctx.get('theme') as ThemeRuntime
    await vi.waitFor(() => { expect(b.describe).toHaveBeenCalled() })
    // The preference is held because the theme is not registered yet.
    expect(theme.getTheme().preference).toBe('system')
