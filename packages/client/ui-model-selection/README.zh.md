...
- **推理强度记忆按 Host 隔离**——记住的推理强度存于 Host 设置文档，随模型跨会话与重启存活，但不会跨不同 Host 或用户账户跟随。
