...
- `ctx.skills.setInvocationOverride(name, policy | undefined): void` Sets or clears a global runtime invocation override for one skill name. The override wins over every provider-declared policy at list and get projection; passing `undefined` restores the provider policy, and a name with no catalog entry is accepted and applies once such a skill appears. Overrides live in the registry's global layer only. [`dsh-host-skill-manager`](../../host/skill-manager/README.md) persists them through the `skill-overrides` settings namespace.
...
- **Overrides are global and explicit** — `setInvocationOverride` has no per-scope variant and no "clear all"; the management Remote writes complete policies, so a touched skill keeps an override entry even when it equals the provider default.
