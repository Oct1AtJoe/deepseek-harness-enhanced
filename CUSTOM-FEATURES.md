...
| 8. aurora/nebula 主题 | **已插件化（2026-08-16）**：主题定义移入新包 `ui-theme-custom`，经官方 `ctx.theme.register()` 注册；官方 ui-theme 还原（BUILTIN_THEMES/inspect/design-platform.css 回上游）。官方保留小型扩展面：`THEME_PREFERENCES`（持久化 schema）、`boot-theme.ts`（启动深色映射）、`AppearanceRow.tsx` 色块、`locales.ts` 名称（共 ~35 行） | 小（扩展面 ~35 行） |
