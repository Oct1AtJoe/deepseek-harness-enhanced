...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/dsh-notification-custom': { kind: 'none', reason: 'Desktop-notification UI over a browser-bound session projection; registers nothing model-facing and writes no session event.' },
  ...
  'packages/client/ui-kanye-pet': { kind: 'none', reason: 'Browser-side UI plugin layer; registers nothing model-facing.' },
  ...
  'packages/client/ui-resend-failed-round': { kind: 'indirect', reason: 'The action re-sends the failed round\'s logged user text through the session-scoped conversation service; the service owns the queued turn and its model-visible effect.' },
  'packages/client/ui-session-reference': { kind: 'indirect', reason: 'The host prompt boundary resolves the mention into readable content and injects the referenced session\'s read-only snapshot; the plugin assembles no provider request.' },
  ...
  'packages/client/ui-theme-custom': { kind: 'none', reason: 'Browser-side tech-theme plugin; registers CSS token sets and a settings row, nothing model-facing.' },
  ...
  'packages/client/ui-settings-subagents': { kind: 'none', reason: 'Browser-side settings surface over the subagentManager Remote; registers nothing model-facing.' },
  ...
  'packages/desktop/kanye-pet': { kind: 'none', reason: 'Bundle-plugin desktop pet overlay; registers nothing model-facing and writes no session event.' },
  ...
  'packages/host/subagent-manager': { kind: 'none', reason: 'Host-side Remote over the Loader backend catalog; appends loader patch text and registers nothing model-facing.' },
